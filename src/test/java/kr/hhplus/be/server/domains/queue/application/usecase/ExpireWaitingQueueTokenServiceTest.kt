...
class ExpireWaitingQueueTokenServiceTest : BehaviorSpec({
    ...
    given("대기 큐가 존재하는 스케줄들이 있을 때") {
        ...
        every { waitingQueueRepository.removeExpiredWaiting(any()) } just runs

        `when`("만료된 대기열 엔트리를 정리하면") {
            expireWaitingQueueTokenService.execute()

            then("스케줄별 대기 큐에서 만료 엔트리를 제거한다") {
                verify(exactly = 1) { waitingQueueRepository.removeExpiredWaiting(1L) }
                verify(exactly = 1) { waitingQueueRepository.removeExpiredWaiting(2L) }
            }
        }
    }
})
